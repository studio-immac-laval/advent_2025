...
Aider le Père Noël à faire sa tournée !

## Démarrer

Compléter la fonction `update` ci-dessous pour que le Père Noël (`santa`) parcourt **automatiquement** toutes les cheminées (`chimneys`).
...
def init() :
    ...
    santa = {
        "x" : randint(10, 118),
        "y" : randint(10, 118),
        "target" : -1,              # L'indice de la cheminée cible dans la liste chimneys (-1 : pas de cible)
        "visiting" : 0,             # Est-ce que le Père Noël est dans une cheminée (0 : non, -1 : il descend, 1 : il remonte)
        "radius" : 3                # La taille du Père Noël (à réduire quand il rentre dans une cheminée)
    }
